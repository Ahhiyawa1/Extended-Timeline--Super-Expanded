...
PixelShader = 
{
	...
	[[
		float3 CalcWaterNormal( float2 uv, float vTimeSpeed )
		{
			...
			float3 noiseNormal1 = tex2D( WaterNoise, noiseScale + 3.0f ).rgb;
			float3 noiseNormal2 = tex2D( WaterNoise, noiseScale + 3.0f ).rgb;		
			float3 normalNoise = noiseNormal1 + noiseNormal2;
	...
	[[
		float3 CalcWaterNormal( float2 uv, float vTimeSpeed )
		{
			...
			float2 time1 = vScaledTime * float2( 0.0f, 0.0f );
			float2 time2 = -vScaledTime * 0.75f * float2( 0.0f, 0.0f );
			...
			float3 normalNoise = noiseNormal1 + noiseNormal2 + float3( 0.0f, 0.0f, 0.0f );
